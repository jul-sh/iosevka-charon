...
def get_metrics(font_path):
    ...
    return {
        ...
        "total_height": font["hhea"].ascender
        - font["hhea"].descender
        + font["hhea"].lineGap,
...
def main():
    ...
    before_regular = Path(
        "general_use_fonts/IosevkaCharonMono/ttf/IosevkaCharonMono-Regular.ttf"
    )
    ...
    print(
        f"  Ascender:    {after_metrics['ascender']:>5} ({after_metrics['ascender'] - before_metrics['ascender']:+d})"
    )
    print(
        f"  Descender:   {after_metrics['descender']:>5} ({after_metrics['descender'] - before_metrics['descender']:+d})"
    )
    print(
        f"  LineGap:     {after_metrics['lineGap']:>5} ({after_metrics['lineGap'] - before_metrics['lineGap']:+d})"
    )
    print(
        f"  WinAscent:   {after_metrics['winAscent']:>5} ({after_metrics['winAscent'] - before_metrics['winAscent']:+d})"
    )
    print(
        f"  WinDescent:  {after_metrics['winDescent']:>5} ({after_metrics['winDescent'] - before_metrics['winDescent']:+d})"
    )
    print(
        f"  Total:       {after_metrics['total_height']:>5} ({after_metrics['total_height'] - before_metrics['total_height']:+d}, {((after_metrics['total_height'] - before_metrics['total_height']) / before_metrics['total_height'] * 100):.1f}%)"
    )
    ...
    height_increase_pct = (
        (after_metrics["total_height"] - before_metrics["total_height"])
        / before_metrics["total_height"]
        * 100
    )
...
éttlætis lýst, ΔΕΚΕΜΒΡΙΟΥ Ἐπειδὴ ἡ ἀναγνώριση τῆς ἀξιοπρέπειας, ποὺ εἶναι σύμφυτη"""
    ...
    if before_complex and after_complex:
        ...
        combined = Image.new(
            "RGB",
            (max_width, before_complex.height + after_complex.height + 20),
            "white",
        )
    ...
    if before_multi and after_multi:
        ...
        combined = Image.new(
            "RGB", (max_width, before_multi.height + after_multi.height + 20), "white"
        )
