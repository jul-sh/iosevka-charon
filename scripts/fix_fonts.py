...
from autokern import add_auto_kerning
...
_VERSION_JSON = Path(__file__).resolve().parent.parent / "sources" / "version.json"
_cached_version: dict | None = None


def load_gf_version() -> dict:
    """Load the Google Fonts version info from sources/version.json.

    Returns a dict with keys: upstream, gf_version, gf_version_string.
    Run scripts/sync_version.py to generate this file.
    """
    ...
    if not _VERSION_JSON.exists():
        raise FileNotFoundError(
            f"{_VERSION_JSON} not found. Run 'python3 scripts/sync_version.py' first."
        )

    _cached_version = json.loads(_VERSION_JSON.read_text(encoding="utf-8"))
    return _cached_version
...
def fix_font_revision(font: TTFont) -> bool:
    """Fix font revision, nameID3, and nameID5 to the Google Fonts compliant version.

    Reads the version from sources/version.json (generated by sync_version.py).
    Sets:
      - head.fontRevision to the GF float (e.g. 33.305)
      - nameID 5 to "Version 33.305"
      - nameID 3 to "33.305;UKWN;PostScriptName" (GF unique ID format)
    """
    ver = load_gf_version()
    gf_version = float(ver["gf_version"])
    gf_version_str = ver["gf_version"]
    version_string = ver["gf_version_string"]

    font["head"].fontRevision = gf_version
    ...
    # ID 3: Unique identifier — format: "version;vendorID;PostScriptName"
    vendor_id = font["OS/2"].achVendID.strip() if "OS/2" in font else "UKWN"
    ps_name_rec = name_table.getName(6, 3, 1, 0x409)
    ps_name = ps_name_rec.toUnicode() if ps_name_rec else "Unknown"
    unique_id = f"{gf_version_str};{vendor_id};{ps_name}"
    name_table.setName(unique_id, 3, 3, 1, 0x409)
    name_table.setName(unique_id, 3, 1, 0, 0)

    logger.info(f"  ✓ Fixed font revision: {gf_version} ({version_string})")
    logger.info(f"  ✓ Fixed unique ID (ID 3): {unique_id}")
    return True
